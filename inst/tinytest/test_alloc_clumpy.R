...
gart_results_many <- gart_cpp(P100, c(10L, 20L))
...
res_stay <- gart_cpp(P_stay, c(1L, 2L, 3L))
...
expect_equal(gart_cpp(P_neg, c(1L, 2L)), 2L)

# --- sample_lognorm_area_cpp() ----------------------------------------------

set.seed(1L)
a <- sample_lognorm_area_cpp(area_mean = 4, area_var = 2)
expect_true(a >= 1L)
expect_true(is.integer(a))

# With zero or NA mean, should return 1
expect_equal(sample_lognorm_area_cpp(0, 1), 1L)
expect_equal(sample_lognorm_area_cpp(NA_real_, 1), 1L)

# --- raster_neighbors_cpp() -------------------------------------------------

nbrs <- raster_neighbors_cpp(3L, 4L) # 3-row, 4-col raster (12 cells)
...
probs <- rep(0.8, n)
nbrs <- raster_neighbors_cpp(4L, 4L)
