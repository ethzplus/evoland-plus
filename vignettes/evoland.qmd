...
unlink("arealstatistik-model.evolanddb", recursive = TRUE)
```

This file introduces the `evoland-plus` R package, or `evoland` for short.
In brief, `evoland-plus` adheres to the idea that the likelihood of land use change in a given location can be described based on historical patterns, augmented with assumptions about the development socio-economic and environmental drivers of land use change.

We'll be following a three-phase workflow:
...
In order to read in data, we first must declare the spatiotemporal structure which will serve as the framework for all our modelling:
...
- **Metadata** describing the land use/land cover classes we will use

## Setup

First, we load the package and create a `evoland_db` object: unlike most R objects like `data.frame`s, this is a **mutable** object[^r6], meaning we can apply actions to it.
This is the wrong place to go into details, but the gist of this style of object is that it provides a nice representation of a durable database on disk.
...
csv_file <-
  ...
  (\(x) x[["Name"]])() |>
  grep(pattern = ".csv", x = _, value = TRUE)
...
Now that we know which coordinates we have fundamental data for, let's use that information to reduce the amount of coordinates we'll go forward with:

```{r}
#| label: pare-down-coords
...
For this domain in the middle of Switzerland, this represents a small amount of savings -
but as the sparsity of the domain grows, e.g. by omission of adjacent administrative areas,
or by omission of certain land use classes - the advantage of the tabular representation becomes clearer.
