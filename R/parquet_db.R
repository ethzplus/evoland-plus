...
# Resolves which columns to use for autoincrement, map, key: pre-existing schema takes precedence
resolve_cols <- function(x, metadata = list(), col_function = character(1)) {
  cols_metadata <- metadata[[col_function]] # comes as joint csv string
  cols_attr <- attr(x, col_function)

  if (!is.null(cols_metadata)) {
    if (!is.null(cols_attr) && !setequal(cols_metadata, cols_attr)) {
      warning(glue::glue(
        "Metadata for {col_function} ({cols_metadata_joint}) takes precedence over",
...
# Resolves the partitioning scheme to use; pre-existing schema takes precedence
resolve_partition_clause <- function(x, metadata) {
  cols_metadata <- metadata[["partition_cols"]] # comes as joint, quoted csv string
  cols_attr <- attr(x, "partition_cols")

  if (!is.null(cols_metadata)) {
    if (!setequal(cols_metadata, cols_attr)) {
      warning(glue::glue(
        "Metadata for {col_function} ({toString(cols_metadata)}) takes precedence over",
        "attributes ({toString(cols_attr)}) for this commit"
      ))
    }
    out <- cols_metadata
  ...
  paste0(
    ", partition_by (",
    toString(glue::glue('"{out}"')),
    ")"
  )
}

# Resolves the metadata to commit; only new metadata can be added
