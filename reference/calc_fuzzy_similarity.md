...
Calculates fuzzy similarity indices between two categorical rasters
using a moving window approach with optional exponential decay. This
allows for spatial tolerance when comparing maps, recognizing that
perfect pixel-by-pixel match may be too strict for spatial models.
...
The exponential decay function is: exp(-d / decay_divisor) where d is
the Euclidean distance from the center cell.
